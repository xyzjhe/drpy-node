...
# 本地资源管理.py - 借鉴FileExplorer.php的数据库优化方案
# 整合了JSON本地读取.py的解析逻辑 - 增强版
# 添加了在线直播大分类功能（放在最后）
...
class Spider(Spider):
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if not is_root and parent and parent != path:
            ...
            self.log(f"返回上一级: {path} -> {parent} (显示名称: {parent_name})")
            ...
            self.log(f"当前是根目录，不显示返回上一级: {path}")
        # ===== 返回上一级逻辑结束 =====
    ...
    def detailContent(self, ids):
        ...
        if self.is_image_file(ext):
            ...
            self.log(f"音频文件播放串: {vod['vod_play_url']}")
        elif self.is_media_file(ext):
            url = f"file://{id_val}"
            display_name = os.path.splitext(name)[0]
            vod['vod_play_url'] = f"{display_name}${url}"
            vod['vod_pic'] = self.file_icons['video']
            self.log(f"视频文件播放串: {vod['vod_play_url']}")
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # 如果id包含$符号，格式可能是 "线路名$url" 或 "电视台名$url"
        if '$' in id:
            parts = id.split('$', 1)
            if len(parts) == 2:
                # 提取真正的URL
        ...
        if url.startswith('file://'):
            ...
            if os.path.exists(file_path) and self.is_audio_file(self.get_file_ext(file_path)):
                ...
                if lrc:
                    result["lrc"] = lrc
                    self.log(f"✅ 歌词已添加: {os.path.basename(file_path)}")
